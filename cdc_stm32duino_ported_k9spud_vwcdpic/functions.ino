...
//-----------------------------------------------------------------------------
/*!
  \brief    void SendByte(uint8_t byte_u8)
...
//based on ATcommand lib

void BT_PLAY_PAUSE() {
  #ifdef BLUETOOTH
  //mySerial.print(sBT_PLAY_PAUSE);
  if(bluetooth.sendCommand("AT+MV","MB",500)){ //MB = playing
    BT_PAUSE();
  } else {
   BT_PLAY();
  } 
...
void BT_PAUSE() {
  #ifdef BLUETOOTH
  //mySerial.print(sBT_PLAY_PAUSE);
  if (!bluetooth.sendCommand("AT+MA","MA",500)) Serial.println(F("Command AT+MA failed!"));
  #endif
}
...
void BT_PLAY() {
  #ifdef BLUETOOTH
  //mySerial.print(sBT_PLAY_PAUSE);
  if (!bluetooth.sendCommand("AT+MA","MB",500)) Serial.println(F("Command AT+MA failed!"));
  #endif
}
...
void BT_STOP() {
  #ifdef BLUETOOTH
  //mySerial.print(sBT_PLAY_PAUSE);
  if (!bluetooth.sendCommand("AT+MC","OK",500)) Serial.println(F("Command AT+MC failed!"));
  #endif
}

void BT_PREVIOUS() {
  #ifdef BLUETOOTH
  //mySerial.println(sBT_PREVIOUS);
  if (!bluetooth.sendCommand("AT+ME","OK",1000)) Serial.println(F("Command AT+ME failed!"));
  #endif
}

void BT_NEXT() {
  #ifdef BLUETOOTH
  //mySerial.println(sBT_NEXT);
   if (!bluetooth.sendCommand("AT+MD","OK",1000)) Serial.println(F("Command AT+MD failed!"));
  #endif
}
...
void BT_FF(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+MF",1000)) Serial.println(F("Command FF failed!"));
  #endif
}

void BT_RW(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+MH",1000)) Serial.println(F("Command FF failed!"));
  #endif
}

void BT_PAIR(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+CA",1000)) Serial.println(F("Command enter pairing failed!"));
  #endif
}

void BT_ENDPAIR(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+CB",1000)) Serial.println(F("Command endpairing failed!"));
  #endif
}


void BT_CONNECT_TO_LAST_PAIRED_DEVICE(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+CC",1000)) Serial.println(F("Command connect to last paired device failed!"));
  #endif
}
...
void BT_DISCONNECT(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+CD",1000)) Serial.println(F("Command BT disconnect failed!"));
  #endif
}
...
void BT_ANSWER_CALL(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+CE",1000)) Serial.println(F("Command answer call failed!"));
  #endif
}

void BT_REJECT_CALL(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+CF",1000)) Serial.println(F("Command reject call failed!"));
  #endif
}

void BT_HANG_UP(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+CG",1000)) Serial.println(F("Command hang up failed!"));
  #endif
}

void BT_REDIAL(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+CH",1000)) Serial.println(F("Command redial failed!"));
  #endif
}

void BT_VOLUME_UP(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+CK",1000)) Serial.println(F("Command bluetooth volue up failed!"));
  #endif
}

void BT_VOLUME_DOWN(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+CL",1000)) Serial.println(F("Command bluetooth volume down failed!"));
  #endif
}

void BT_GET_ADR(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+MR",1000)) Serial.println(F("Command get bt address failed!"));
  #endif
}

void BT_GET_PIN(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+MP",1000)) Serial.println(F("Command get pin failed!"));
  #endif
}

void BT_GET_NAME(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+MN",1000)) Serial.println(F("Command get bt name failed!"));
  #endif
}

void BT_GET_SW(){
  #ifdef BLUETOOTH
   if (!bluetooth.sendCommand("AT+MQ",1000)) Serial.println(F("Command get bt version failed!"));
  #endif
}
